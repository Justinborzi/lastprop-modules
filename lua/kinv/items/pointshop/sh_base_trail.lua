...
if (SERVER) then
	function ITEM:Think()
		if (IsValid(self.trailEnt)) then
			if ( hook.Run("PS2_VisualsShouldShow", self:GetOwner()) == false and not self.trailEnt:GetNoDraw()) then
				self.trailEnt:SetNoDraw(true)
			elseif ( hook.Run("PS2_VisualsShouldShow", self:GetOwner()) != false and self.trailEnt:GetNoDraw()) then
				self.trailEnt:SetNoDraw(false)
			end
		end
	end
